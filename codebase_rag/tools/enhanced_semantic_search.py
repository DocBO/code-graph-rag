...
def create_enhanced_semantic_search_tool(
    repo_path: str | None = None, console: Console | None = None
) -> Tool:
    """
    ...
    async def enhanced_semantic_search(query: str, top_k: int = 10) -> str:
        ...
        """
        logger.info(
            f"[Tool:EnhancedSemanticSearch] Searching for: '{query}' (top_k={top_k})"
        )

        if not has_semantic_dependencies():
            return (
                "❌ Semantic search is not available.\n\n"
                "This requires an external embedder and Qdrant:\n"
                "1. Set EMBED_ENDPOINT, EMBED_MODEL (and optionally EMBED_API_KEY) in .env\n"
                "2. Install the Qdrant client: pip install '.[semantic]' (qdrant-client)\n\n"
                "To enable:\n"
